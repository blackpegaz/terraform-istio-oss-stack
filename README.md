...
| <a name="input_jaeger_operator_instance_allinone_affinity"></a> [jaeger\_operator\_instance\_allinone\_affinity](#input\_jaeger\_operator\_instance\_allinone\_affinity) | Map of objects used to configure affinity rules for the Jaeger AllInOne instance.<br><br>  Example:<br>    {<br>    "nodeAffinity": {<br>      "requiredDuringSchedulingIgnoredDuringExecution": {<br>        "nodeSelectorTerms": [<br>          {<br>            "matchExpressions": [<br>              {<br>                "key": "kubernetes.io/os",<br>                "operator": "In",<br>                "values": [<br>                  "linux"<br>                ]<br>              }<br>            ]<br>          }<br>        ]<br>      }<br>    },<br>  } | `map(any)` | `{}` | no |
